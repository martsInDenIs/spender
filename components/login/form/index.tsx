...
export default function LoginForm({ onSubmit }: Props) {
    ...
    return <form onSubmit={handleSubmit(submitHandler)} className='flex flex-col gap-2'>
        <Checkbox {...register('role', { required: true })} value={ROLE.PARENT} label={t('login.form.sugarDM')} />
        <Checkbox {...register('role', { required: true })} value={ROLE.CHILD} label={t('login.form.sugarPartner')} />
